...
def perm_event_calculator(filename, chain_id, ref_z_1_order, ref_z_2_order, ref_xy_1_order, ref_xy_2_order, chains_letter, pore_radius = 9.5):
    n_ref_atoms = 3
    ...
    for atom in compendio_atomos:
        ...
        for frame in range(1,total_frames):
            ...
            if is_in_cylinder_now and not is_in_cylinder_prev:
                ...
                if not ingreso_egreso_lateral(atoms_coordinates, frame, atom, Pore, ingreso=True):
                    entro = True
                    frame_entrada = frame
            ...
            if is_in_cylinder_prev and not is_in_cylinder_now and entro:
                ...
                if not ingreso_egreso_lateral(atoms_coordinates, frame, atom, Pore, ingreso=False):
                    if ingreso != salida:
                        n_eventos += 1
                        frame_salida = frame
